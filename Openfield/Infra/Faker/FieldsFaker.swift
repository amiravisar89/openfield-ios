...
struct FieldsFaker: FakerProvider {
    let faker: Faker

    var fieldIds: [Int] {
        return createFieldIds()
    }
    ...
    func createFields() -> [Field] {
        var result: [Field] = []
        for id in fieldIds {
            result.append(createField(id: id))
        }
        return result
    }

    func createImageGroup(id: Int, fieldName: String) -> FieldImageGroup {
        return FieldImageGroup(fieldId: id,
                               fieldName: fieldName,
                               imageryMainImage: randomElement(list: urls),
                               imageryMainType: .rgb,
                               date: Date(),
                               bounds: tagBounds,
                               imagesByLayer: createImagesByLayer(),
                               region: Region.local,
                               sourceType: .plane)
    }

    private func createImagesByLayer() -> [AppImageType: [PreviewImage]] {
        var result: [AppImageType: [PreviewImage]] = [:]
        for type in AppImageType.allCases {
            result[type] = [PreviewImage(url: randomElement(list: urls), height: 50, width: 50, imageId: 0, issue: Issue(comment: "", isHidden: true, issue: .empty)), PreviewImage(url: randomElement(list: urls), height: 50, width: 50, imageId: 0, issue: Issue(comment: "", isHidden: true, issue: .empty))]
        }
        return result
    }

    private func createFieldIds() -> [Int] {
        var fieldsIdsArray = [Int]()
        for _ in 1 ... faker.number.randomInt(min: 10, max: 100) {
            fieldsIdsArray.append(faker.number.randomInt(min: 5, max: 500_000))
        }
        return fieldsIdsArray
    }

    private func createImageGroups(count: Int, id: Int, fieldName: String) -> [FieldImageGroup] {
        var result = [FieldImageGroup]()
        for _ in 1 ... count {
            result.append(createImageGroup(id: id, fieldName: fieldName))
        }
        return result
    }

    private func createField(id: Int) -> Field {
        let fieldName = faker.address.county()
        return Field(id: id,
                     country: "US",
                     name: fieldName,
                     farmName: faker.address.city(),
                     dateUpdated: faker.date.backward(days: 365),
                     imageGroups: createImageGroups(count: faker.number.randomInt(min: 1, max: 15), id: id, fieldName: fieldName),
                     coverImage: nil,
                     region: Region.local,
                     farmId: faker.number.randomInt(min: 1, max: 1000),
                     filters: [],
                    subscriptionTypes: [])
    }
