...
struct ImageryFaker: FakerProvider {
    let faker: Faker
    var fields: [Field] = []

    private let imageryTimeAgo: Date = Calendar.current.date(byAdding: .month, value: -5, to: Date())!
    private let imageryNow: Date = .init()

    init(faker: Faker) {
        self.faker = faker
    }

    func createImagery() -> Imagery {
        let field = randomElement(list: fields)
        let imagesNumber = faker.number.randomInt(min: 1, max: 30)
        return Imagery(images: createImageryImageList(count: imagesNumber, field: field),
                       date: faker.date.between(imageryTimeAgo, imageryNow), region: Region.current,
                       isRead: faker.number.randomBool())
    }

    private func createImageryImageList(count: Int, field: Field) -> [ImageryImage] {
        var result: [ImageryImage] = []
        for _ in 1 ... count {
            result.append(createImageryImage(field: field))
        }
        return result
    }

    private func createImageryImage(field: Field) -> ImageryImage {
        return ImageryImage(url: randomElement(list: urls), date: Date(), region: Region.local, layer: .rgb,
                            field: field)
    }
